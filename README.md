...
- 评论编号、正文、时间、引用关系、是否为洞主，以及树洞已提供的 Alice、Bob、A君、B君等匿名标签。

以下内容不会进入公开快照：

- 监控设置、扫描状态、错误详情和任何登录凭据；
- 真实身份信息、身份类型、专属标识、媒体 ID、图片和媒体 URL；匿名标签不属于真实身份字段，会原样保留；
- 已删除、隐藏或受保护的内容；
...
Vercel Web Analytics 已在项目中启用。公开页面通过同源的 `/_vercel/insights/script.js` 上报匿名访问统计；可在 Vercel 项目的 **Analytics** 页面查看访问量、页面、来源和设备等汇总数据。
